...
2. 回到 DSHA：配置页点「**🔐 ADB 无线配对**」
3. 手机系统：**设置 → 开发者选项 → 无线调试** → 打开
4. 点「**使用配对码配对设备**」→ 记下屏幕上的 **6 位配对码**
4. 在通知栏找到配对提示，通知栏卡片展开 → 点「输码配对」→ 输入 6 位码
5. 配对成功会显示 ✅，自动获得 `uid=2000(shell)` 设备权限
...
| 安装⑥时会添加一个插件，会向agent的系统提示末注入一段引导模型使用shell的提示词，👉极简模式暂不支持注入提示词👈 |
