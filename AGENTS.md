...
- **3081 局域网桥转发请求头必须补齐结尾空行，且 chunk 上限不能掐死 1MB.** 局域网桥（`LanProxyService`）有两个深坑：① `rewriteHost` 切分请求头重拼时，Java `split` 丢弃了结尾空行；如果在末尾补加 `X-Dsha-Token` 时没有显式追加 `\r\n`，发给后端的请求头会缺失 HTTP 协议结尾的 `\r\n\r\n`，Node.js parser 一直等头部结束，代理端一直等响应，形成双向死锁导致局域网访问完全超时卡死；② `pipeChunked` 转发分块响应时，`size == 0`（`0\r\n`）必须连带读出并转发紧随其后的 CRLF，且单块上限 `MAX_CHUNK` 绝不能写死 1MB —— 大型插件（如 1.22MB 的 `dsh-better-sidebar/client.js`）以单块 >1MB 下发时会被误判为非法超大块直接掐断连接，导致脚本传输残缺、前端永远停留在 `Loading plugins…` 旋转等待。
- **通知栏必须划分独立结果通道，且前台决不能用 `FOREGROUND_SKIP` 吞掉通知.** 早期实现将运行中实时步骤（`NOTIF_TASK_RUNNING=2003`）、完成通知（`NOTIF_TASK=2002`）与紧急终止（`NOTIF_TASK_STOPPED=2004`）全部混在 `dsh_agent_channel` 中。由于工具调用（`tool/call`）每一步都更新运行中通知，若与完成通知同通道会导致高频横幅/振动打扰；而 `HttpShellService` 在前台检测到 `appInForeground` 时若直接 `return "FOREGROUND_SKIP"`，会把报错（如 fetch failed/token 超限）和完成通知强行拦截只弹 Toast，导致通知栏被清空且丢失 RemoteInput 原生输入框。必须将结果类通知独立划入 `Constants.CHANNEL_TASK_RESULT` (`dsh_task_result_channel`)，并坚持「先写入系统通知栏，前台再追加 Toast」的非阻断时序。
- **任务中断必须全链路闭环，区分通知栏制动与 WebUI 停止.** 用户在通知栏点「🛑 停止任务」由 `ConfirmReceiver` 主动发 `2004` 终止通知并注入 `.cancel_requested`，插件调用 `agent.cancel()`；而用户在 Web 页面点停止或模型异常中断时，后端抛出 `turn/end` (reason: `aborted`)。`dsh-task-notifier` 插件绝不能简单对 `aborted` 一刀切 `return`（否则 Web 界面停止将没有任何通知提示），需通过短时间戳判断是否为通知栏触发：非通知栏触发时主动补发 `DSHA · 任务已中断` 通知，实现多端停止行为的无缝统一。
