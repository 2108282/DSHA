...
### 4.2 澎湃 OS 灵动岛双圆钮决策与安全授权规范（针对有限窄格子定制）

在小米澎湃 OS 的模板 ②（双圆钮决策卡片）中，右侧实心绿勾与红叉占据近 1/3 宽度，左侧文字区域极窄（单行容纳 10~14 字）。**严禁将整句长废话重复填充到上下两行！** 上行统一提炼为核心事件大标题，下行直击核心目标与动作：

| 序号 | 触发场景 | 上行大标题 (提炼核心主题 · 6~8字) | 下行正文 (具体授权对象与动作 · 5~12字) | 胶囊右耳 |
| :---: | :--- | :--- | :--- | :---: |
| **1** | **10分钟免打扰租约** | **`危险权限授权申请`** | **`申请 10分钟免打扰租约`** | `危险授权` |
| **2** | **Shell 危险命令** | **`特权命令执行确认`** | 纯命令原文（如 `rm -rf test_dir`） | `命令确认` |
| **3** | **高危系统指令** (reboot/dd/wipe) | **`高危系统指令确认`** | 纯命令原文（如 `reboot`） | `命令确认` |
| **4** | **读屏 / 无障碍操作** | **`屏幕操作授权申请`** | **`读取当前屏幕文字与控件`** | `屏幕授权` |
| **5** | **应用敏感 / 支付操作** | **`应用敏感操作确认`** | **`微信: 点击确认支付`** | `敏感确认` |
| **6** | **文件覆盖 / 业务决策** | **`确认本地文件修改`** | **`覆盖 index.ts？`**（核心文件名与动作） | `等待决策` |
...
* **锁屏置顶实时卡片 & 下拉通知栏卡片**：
  ...
  └────────────────────────────────────────────────────────┘
  ```
  > *注：系统在 长久留存（2 分钟）后自动将该卡片从状态栏及锁屏上平滑收起消除。*
...
### 3. 【助手提问 (ask_user)】（强提醒交互：拦截与秒切提问态）
...
1. **小米澎湃 OS (模板 ① 日程单按钮规范)**：
   ...
   * **展开纯黑卡片**：
     ...
     ├────────────────────────────────────────────────────────┤  <-- 1dp 极细半透明分割线
     │  等待回答                                              │  <-- 蓝色状态小标
     │  智能体正在等待你的回答与选择...      ( 💬 返回对话 )   │  <-- 底部：提取自定义提问内容 + [返回对话] 按钮
     └────────────────────────────────────────────────────────┘
     ```
   * **防误触发机制**：
     在 `task-notifier`、`status-overlay` 插件与 `appOverlay` Java 接口中进行 4 重防线过滤。精准匹配中文 `s.contains("等待回答")`，彻底剔除英文单词 `"task" / "subtask"` 包含 `"ask"` 引发的错误匹配拦截。
