...
def main():
    ...
    output = Path(__file__).resolve().parents[1] / 'app/build/rc13-update-fixtures'
    ...
    for name in ['valid', 'wrong-signature', 'wrong-package', 'wrong-flavor']:
        ...
        if name == 'valid':
            with zipfile.ZipFile(unsigned, 'a', compression=zipfile.ZIP_STORED) as archive:
                archive.writestr('assets/download-progress-fixture.bin', bytes(8 * 1024 * 1024))
