...
if [ -d "$REPO_ROOT/app/src/main/assets/dsh-web-mobile" ]; then
  mkdir -p "$ROOTFS_DIR/root/patches/builtin"
  cp -r "$REPO_ROOT/app/src/main/assets/dsh-web-mobile" "$ROOTFS_DIR/root/patches/builtin/" 2>/dev/null || true
