...
# 若本地未找到底包，自动从官方 Release 0.1.7-rc.2 下载纯净底包
if [ -z "$TAR_SRC" ]; then
    echo "--> 未检测到本地底包，正在自动从官方 Release (0.1.7-rc.2) 下载纯净底包..."
    DOWN_URL="https://github.com/2108282/DSHA-FR/releases/download/0.1.7-rc.2/rootfs.tar.gz"
    ...
    if command -v gh >/dev/null 2>&1; then
        ...
        if gh release download 0.1.7-rc.2 --repo "${GITHUB_REPOSITORY:-2108282/DSHA-FR}" --pattern "rootfs.tar.gz" --dir /tmp/dsha_download 2>/dev/null && [ -s "$DL_FILE" ]; then
            DL_OK=1
        fi
    fi
