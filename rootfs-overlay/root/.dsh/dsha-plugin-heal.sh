...
# 2.5 永久自愈 Office 原生转码服务与字库依赖
SRC="/usr/local/lib/node_modules/@deepseek-ai/dsh/node_modules/@deepseek-ai"
for m in dsh-office-to-pdf libreoffice-kit libreoffice-kit-wasm; do
  [ -d "$SRC/$m" ] || continue
  mkdir -p "/root/.dsh/profiles/node_modules/@deepseek-ai"
  ln -sfn "$SRC/$m" "/root/.dsh/profiles/node_modules/@deepseek-ai/$m"
  mkdir -p "/root/.dsh/profiles/web/node_modules/@deepseek-ai"
  ln -sfn "$SRC/$m" "/root/.dsh/profiles/web/node_modules/@deepseek-ai/$m"
  mkdir -p "/usr/local/lib/node_modules/@deepseek-ai"
  ln -sfn "$SRC/$m" "/usr/local/lib/node_modules/@deepseek-ai/$m"
done
