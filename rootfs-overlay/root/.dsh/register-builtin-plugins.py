...
def ensure_office_services():
    """保障 DSH 原生 Office 预览核心依赖与字库服务永久自愈：
    1. 自动挂载 Android 宿主系统字体至 /usr/share/fonts/android；
    2. 确保 @deepseek-ai/dsh-office-to-pdf, libreoffice-kit, libreoffice-kit-wasm
       在 profiles/node_modules, profiles/web/node_modules, /usr/local/lib/node_modules 建立持久软链；
    3. 确保 profiles/web/cordis.patch.yml 中包含 office-to-pdf 插件激活项。
    """
    count = 0
    # 1. 宿主字库保障
    font_target = local('/usr/share/fonts/android')
    if os.path.isdir('/system/fonts') and not os.path.ismount(font_target):
        try:
            os.makedirs(font_target, exist_ok=True)
            os.system('mount -o bind /system/fonts "%s" 2>/dev/null' % font_target)
        except OSError:
            pass

    # 2. 插件符号链接自愈
    src_base = local('/usr/local/lib/node_modules/@deepseek-ai/dsh/node_modules/@deepseek-ai')
    dest_bases = [
        local(os.path.join(DSH_HOME, 'profiles', 'node_modules', '@deepseek-ai')),
        local(os.path.join(DSH_HOME, 'profiles', 'web', 'node_modules', '@deepseek-ai')),
        local('/usr/local/lib/node_modules/@deepseek-ai'),
    ]
    modules = ['dsh-office-to-pdf', 'libreoffice-kit', 'libreoffice-kit-wasm']
    for m in modules:
        src = os.path.join(src_base, m)
        if not os.path.isdir(src):
            continue
        for dest_base in dest_bases:
            os.makedirs(dest_base, exist_ok=True)
            target = os.path.join(dest_base, m)
            if os.path.islink(target):
                try:
                    if not os.path.exists(target) or os.path.realpath(target) != os.path.realpath(src):
                        os.unlink(target)
                    else:
                        continue
                except OSError:
    ...
    # 3. cordis.patch.yml 插件激活自愈
    patch_file = local(PATCH_FILE)
    if os.path.isfile(patch_file):
        try:
            with open(patch_file, 'r', encoding='utf-8') as f:
                content = f.read()
            if 'office-to-pdf' not in content:
...
def register():
    ...
    try:
        ...
        o_linked = ensure_office_services()
        if o_linked: lines.append('已保障 Office 预览核心依赖就绪（%d 处链接）' % o_linked)
