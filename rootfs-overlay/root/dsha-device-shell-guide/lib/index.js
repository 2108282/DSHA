...
const PROMPT = [
  ...
  '  2) Android 框架接口：走 127.0.0.1:3095/app/*（无障碍读屏/点按/中文输入、通知栏、剪贴板、传感器等，由 Root 自动守护）。',
  '  3) 宿主特权通道：容器终端内已直通 am、pm、cmd、input、screencap、dumpsys 命令（亦可走 127.0.0.1:3095/exec?cmd=）。',
  '     例：启动应用首选用 /app/launch?pkg= 或直接敲 am start；读屏点按用 /app/ui/dump 和 /app/ui/tap。',
  '',
  '■ 完整端点清单（读屏 / 点按 / 输入 / 截屏 / 通知 / 剪贴板 / 传感器 / 导出文件 …）：',
  '  T=$(cat /root/.dsh/.bridge_token)',
  '  curl -s "http://127.0.0.1:3095/app/help?token=$T"',
  '  → 需要具体端点参数时查一次，最小常用清单：',
