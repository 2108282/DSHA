...
| CI gatekeepers | Every push runs Fast checks: manifest consistency, offline signature verification, 67 pure-logic assertions, and real compilation of every asset script. Releases abort outright on a certificate fingerprint mismatch |
...
| Pure-logic test suite | 67 assertions with no Android dependency; `bash tools/pure-logic-test.sh` finishes in seconds |
...
bash tools/pure-logic-test.sh   # 67 pure-logic assertions, no device needed
```
