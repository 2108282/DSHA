...
public final class WebDownloadModel extends AndroidViewModel {
    public static final class State {
        ...
        public State(String phase, String message, long bytes, long total) {
            this.phase = phase; this.message = message; this.bytes = bytes; this.total = total;
        }
    ...
    public synchronized void restoreState(Bundle saved) {
        ...
        if (id.matches("[0-9a-fA-F-]{36}\\.part")) {
            ...
            if (candidate.isFile() && candidate.length() == saved.getLong("web-file-size", -1)) {
                ...
                state.setValue(new State("ready", "下载完成，选择保存位置", candidate.length(), candidate.length()));
                return;
            }
        }
        if (!id.isEmpty() || saved.getBoolean("web-file-interrupted"))
            state.setValue(new State("error", "下载被系统中断，请重新导出", 0, -1));
    }
    ...
    public synchronized boolean download(String base, String url, String cookie, String filename,
        ...
        if (!(supplied != null ? WebPreviewPolicy.pageDownload(base, url) : WebPreviewPolicy.sameService(base, url))) {
            close(supplied);
            state.setValue(new State("error", "此下载不是当前本机服务提供的文件", 0, -1));
            return false;
        }
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (!dir.isDirectory() && !dir.mkdirs()) throw new IOException("无法创建下载缓存目录");
    ...
    public synchronized void save(Uri uri) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (count != file.length() || !MessageDigest.isEqual(source.digest(), result.digest()))
                    throw new IOException("保存校验失败，请更换位置重试");
    ...
    public void cancel() {
        ...
        new Thread(() -> { if (c != null) { disconnect(c); } else close(stream); }, "web-download-cancel").start();
    }
