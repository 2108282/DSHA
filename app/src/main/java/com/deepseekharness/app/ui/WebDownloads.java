...
public final class WebDownloads {
    ...
    public WebDownloads(AppCompatActivity activity, Bundle saved) {
        ...
        model.state.observe(activity, state -> {
            ...
            boolean working = "downloading".equals(state.phase()) || "saving".equals(state.phase());
            if (working) {
                if (dialog == null) {
                    dialog = new AlertDialog.Builder(activity)
                            .setTitle("保存网页文件")
                            .setMessage("")
                            .setNegativeButton("取消", (d, w) -> model.cancel())
                            .setCancelable(false)
                            .create();
                }
                ...
                dialog.setMessage(state.message() + count);
                dialog.show();
            } else {
                dismiss();
                if ("ready".equals(state.phase()) && !model.pickerOpen) {
                    ...
                    if ("error".equals(state.phase()) && model.canRetrySave()) {
                        dialog = new AlertDialog.Builder(activity)
                                ...
                                .setNegativeButton("取消", (d, w) -> model.acknowledge())
                                .setCancelable(false)
                                .create();
                        dialog.show();
                    } else {
