...
public final class WebBlobDownload implements AutoCloseable {
    ...
    public WebBlobDownload(WebView view, WebDownloadModel model) {
        this.view = view;
        this.model = model;
    }
    ...
    public void start(String base, String url, String name) {
        ...
        try {
            ...
            String script;
            try (InputStream in = view.getContext().getAssets().open("web-integration/blob.js");
            ...
            view.evaluateJavascript(script, ignored -> {
                if (active != pipe || !WebPreviewPolicy.sameService(base, view.getUrl())) {
                    pipe.fail("网页已离开");
                    return;
                }
                ...
                port.setWebMessageCallback(new WebMessagePort.WebMessageCallback() {
                    @Override public void onMessage(WebMessagePort source, WebMessage message) {
                        ...
                        if (data == null || data.length() > 40000) {
                            pipe.fail("网页文件消息无效");
                            return;
                        }
                ...
                try {
                    port.postMessage(new WebMessage(new JSONObject().put("type", "blob").put("url", url).toString()));
                } catch(Exception e) {
                    pipe.fail("网页文件传输失败");
                }
                checkTimeout(pipe);
            ...
            if (active != null) active.fail("无法打开网页文件");
        }
    }
    ...
    private void accept(Pipe pipe, WebMessagePort source, String json) {
        try {
            ...
            pipe.last = android.os.SystemClock.elapsedRealtime();
            if ("chunk".equals(type)) {
                ...
                pipe.bytes += bytes.length;
                WebTransferPolicy.checkSize(pipe.bytes, -1, WebTransferPolicy.DOWNLOAD_LIMIT, false);
                pipe.output.write(bytes);
                main.post(() -> {
                    if (active == pipe && port == source) {
                        try { source.postMessage(new WebMessage("{\"type\":\"ack\"}")); }
                        catch(Exception e) { pipe.fail("网页已关闭"); }
                    }
                });
                ...
                pipe.complete = true;
                pipe.output.close();
                main.post(() -> { if (port == source) { port.close(); port = null; } });
            } else if ("error".equals(type)) {
                throw new IOException(value.optString("message", "网页文件读取失败"));
            } else {
                throw new IOException("未知网页文件消息");
            }
        } catch(Exception error) {
            pipe.fail(error.getMessage());
        }
    }
    ...
    @Override public void close() {
        if (active != null) active.fail("网页已关闭");
        active = null;
    ...
    private static final class Pipe {
        ...
        volatile long last = android.os.SystemClock.elapsedRealtime();
        long bytes;
        int sequence;
        ...
        Pipe() throws IOException {
            ...
            input = new FilterInputStream(raw) {
                @Override public int read(byte[] b, int o, int n) throws IOException {
                    ...
                    int count = super.read(b, o, n);
                    if (error != null) throw error;
                    if (count < 0 && !complete) throw new IOException("网页文件未传完");
                    return count;
                }
