...
const PROMPT = [
  ...
  '    3) 【双通道协同与闭环汇报】：跨页面/应用直达优先使用 am start (DeepLink/Intent) 并即时核验状态（[EXIT=0]），页面内点击与操作优先使用 /app/ui/*（dump/tap/input，无障碍原生通道，零弹窗）；任务结束统一通过 /app/notify、Toast 和 /app/vibrate 震动向用户交付结果；',
