...
for c in \
  ...
  if [ -f "$c" ]; then
    if grep -q "intentionally not supported" "$c"; then
      TARGETS="$TARGETS $c"
      FAST_OK=1
    elif grep -q 'ALL_INTERFACES_HOST = "0.0.0.0"' "$c" || grep -q "resolveLanTrust" "$c"; then
      # 官方 0.1.5+ 已原生支持 0.0.0.0 绑定，直接标记无需重复 patch 启动项
      ALREADY=1
      FAST_OK=1
    fi
  fi
