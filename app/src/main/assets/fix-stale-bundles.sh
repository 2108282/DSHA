...
# 原则：第三方插件（非 dsh-web-mobile / dsh-device-shell-guide / @deepseek-ai/*）
...
BUILTIN = {'dsh-web-mobile', 'dsh-device-shell-guide', 'dsh-task-notifier',
           'dsh-status-overlay'}
...
def resolvable(name):
    ...
    if os.path.isfile(os.path.join(nm, name, 'package.json')):
        return True
    # DSHA 内置插件实体（/root/dsha-web-mobile 等）——不在 @deepseek-ai 全局，
    # 也不是 nm 下实体（是符号链接）。不认这里会把正常内置插件当 stale 清掉。
    for real in ('/root/dsha-web-mobile', '/root/dsha-device-shell-guide', '/root/dsha-task-notifier',
                 '/root/dsha-status-overlay'):
        if name in ('dsh-web-mobile', 'dsh-device-shell-guide', 'dsh-task-notifier',
